...
void loop() {
  ...
  String message = "";
  SD.begin(chipSelect); // (P_M) Attempt to initialize SD card – placeholder for manual override

  /*
  // Optional SD init check
  if (!SD.begin(chipSelect)) {
    Serial.println("Card failed, or not present");
  } else {
    readNwriteSD(dimBuff); // Read sensor data and write to SD
  }
  */

  // Check if Wi-Fi status has changed
  if (status != WiFi.status()) {
    status = WiFi.status(); // Update status
  ...
  // Check for new client connection to the web server
  ...
  if (client) { // If a new client is available
    ...
    while (client.connected()) { // While client is connected
      delayMicroseconds(10); // Required on Nano RP2040 to avoid SPI starvation

      if (client.available()) { // If data available from client
        char c = client.read(); // Read one character
        currentLine += c;       // Append to current line

        if (c == '\n' && currentLineIsBlank) {
          // HTTP request ends with two newlines
          client.println("HTTP/1.1 200 OK");
          client.println("Content-type:text/html");
          client.println();

          // If it's an AJAX request for sensor data (currently disabled)
          if (currentLine.indexOf("ajax_switch") != -1) {
            // Example placeholder for dynamic sensor feedback
            ;
          } else {
            // Send main HTML interface
            client.println("<html>");
            client.println("<head>");
            ...
            // Render control button interface
            message += "<table> ";
            message += "<tr>";
            ...
            // Velocity input form
            String strvel = "";
            strvel = "<form method=get style=\"font-size:35px;\">Velocity:<input type=number style=\"width:180;height:70;font-size:35px;\" min=0 max=255 size=3 name=b value=";
            strvel.concat(vel);
            strvel = strvel + ">&nbsp;<input name=H type=submit style=\"width:120;height:70;font-size:35px;\" value=INVIA></form>";
            client.print(strvel);

            variabile++;
            String strincr = "";
            strincr.concat(variabile);
            strincr = strincr + "<br>";
        ...
        // Check if the line is blank (newline detection)
        if (c == '\n') {
        ...
        // Parse velocity input from GET request
        ...
        // Handle all movement commands via URL queries
        if (currentLine.endsWith("GET /rover?a=1")) {
          // Diagonal forward-left
          if (scheda == 61) { LevogiroPos(0, 0, 0, scheda); delay(lagTime); LevogiroPos(vel, 2, 1, scheda); }
          else { LevogiroPos(0, 0, 0, scheda); delay(lagTime); LevogiroPos(vel, 1, 2, scheda); }
        }

        if (currentLine.endsWith("GET /rover?a=2")) {
          // Full forward
          LevogiroPos(0, 0, 0, scheda); delay(lagTime); LevogiroPos(vel, 1, 1, scheda);
        }

        if (currentLine.endsWith("GET /rover?a=3")) {
          // Diagonal forward-right
          if (scheda == 61) { LevogiroPos(0, 0, 0, scheda); delay(lagTime); LevogiroPos(vel, 1, 2, scheda); }
          else { LevogiroPos(0, 0, 0, scheda); delay(lagTime); LevogiroPos(vel, 2, 1, scheda); }
        }

        if (currentLine.endsWith("GET /rover?a=4")) {
          // Full left turn
          if (scheda == 61) { LevogiroPos(0, 0, 0, scheda); delay(lagTime); LevogiroPos(vel, 1000, 1, scheda); }
          else { LevogiroPos(0, 0, 0, scheda); delay(lagTime); LevogiroPos(vel, 1, 1000, scheda); }
        }
        ...
        if (currentLine.endsWith("GET /rover?a=6")) {
          // Full right turn
          if (scheda == 61) { LevogiroPos(0, 0, 0, scheda); delay(lagTime); LevogiroPos(vel, 1, 1000, scheda); }
          else { LevogiroPos(0, 0, 0, scheda); delay(lagTime); LevogiroPos(vel, 1000, 1, scheda); }
        }

        if (currentLine.endsWith("GET /rover?a=7")) {
          // Diagonal backward-left
          if (scheda == 61) { LevogiroNeg(0, 0, 0, scheda); delay(lagTime); LevogiroNeg(vel, 2, 1, scheda); }
          else { LevogiroNeg(0, 0, 0, scheda); delay(lagTime); LevogiroNeg(vel, 1, 2, scheda); }
        }

        if (currentLine.endsWith("GET /rover?a=8")) {
          // Full backward
          LevogiroNeg(0, 0, 0, scheda); delay(lagTime); LevogiroNeg(vel, 1, 1, scheda);
        }

        if (currentLine.endsWith("GET /rover?a=9")) {
          // Diagonal backward-right
          if (scheda == 61) { LevogiroNeg(0, 0, 0, scheda); delay(lagTime); LevogiroNeg(vel, 1, 2, scheda); }
          else { LevogiroNeg(0, 0, 0, scheda); delay(lagTime); LevogiroNeg(vel, 2, 1, scheda); }
        }
      }
    }
